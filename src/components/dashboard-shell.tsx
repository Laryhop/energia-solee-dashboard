  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        if (!sData?.summary) throw new Error("Dados da API indisponíveis");
        
        setState({ status: "success", data: { solar: sData, weather: wData } });
      } catch (e) { 
        setState({ status: "error", message: "Erro ao sincronizar com a usina solar." }); 
      }
    }
    loadData();
  }, []);

  if (state.status === "loading") return <div className="flex min-h-screen items-center justify-center dark:bg-[#060d09] dark:text-white font-black animate-pulse text-xl">SINCRONIZANDO USINA...</div>;
  if (state.status === "error") return <div className="flex min-h-screen items-center justify-center text-red-500 font-black px-4 text-center">{state.message}</div>;
    ...
    <main className="min-h-screen bg-[#f8fafc] p-4 transition-colors duration-300 dark:bg-[#060d09] sm:p-8">
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <header className="flex flex-col items-center justify-between gap-6 rounded-[2.5rem] bg-[#0d2a1d] p-8 shadow-2xl md:flex-row border border-emerald-900/50">
          <div className="flex items-center gap-5">
            <div className="rounded-2xl bg-white p-3 shadow-lg">
              <Image src="/solee-logo.png" alt="Logo" width={60} height={60} priority />
            </div>
            <div>
              ...
              <p className="text-xs text-emerald-100/40 font-medium">Dashboard de geração de energia solar.</p>
            </div>
          </div>
          <button onClick={() => {
            const next = !isDark;
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <MetricCard label="Geração Mensal" value={nf.format(solar.summary.monthlyGenerationKwh) + " kWh"} hint={`Meta: ${nf.format(solar.summary.monthlyGenerationKwh)} kWh`} />
          <MetricCard label="Venda Hoje" value={cf.format(solar.summary.economyTodayBrl)} hint="Venda estimada hoje" />
          <MetricCard label="Performance" value={solar.summary.performancePct + "%"} hint={`Tarifa: R$ ${solar.summary.tariffKwhBrl}`} />
          <MetricCard label="Potência Atual" value={nf.format(solar.summary.currentPowerKw) + " kW"} hint="Capacidade L & M" />
          <MetricCard label="Status da Usina" value={solar.summary.statusLabel} hint={solar.summary.location} />
          <MetricCard label="Venda Total" value={cf.format(solar.summary.totalRevenueBrl)} hint="Acumulado histórico" />
          <MetricCard label="Última Leitura" value={new Date(solar.summary.updatedAt).toLocaleTimeString()} hint="Horário de consulta ao backend" />
        </section>
        ...
        <section className="rounded-[2.5rem] bg-white dark:bg-[#0f1d15] p-8 shadow-sm border border-slate-200 dark:border-emerald-900/30 relative">
          <h2 className="text-xl font-black text-[#052e16] dark:text-white mb-2">Geração por hora</h2>
          <p className="text-xs text-slate-500 dark:text-emerald-400/50 font-bold mb-8 italic">Passe o mouse sobre o gráfico para ver quanto foi gerado em cada hora do dia.</p>
          <div className="relative h-[350px] w-full">
            ...
            {hoverData && (
              <div className="absolute z-20 p-4 rounded-2xl bg-[#0d1a12] border-2 border-amber-500 text-white shadow-2xl pointer-events-none transition-all"
                ...
                <p className="text-[9px] mt-1 font-bold text-emerald-400">Geração estimada nessa hora do dia</p>
              </div>
            )}
          </div>
        </section>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <section className="rounded-[2.5rem] bg-white dark:bg-[#0f1d15] p-8 border border-slate-200 dark:border-emerald-900/30">
            <h2 className="text-xl font-black mb-4 dark:text-white">Valor estimado da energia gerada</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-6">Estimativa em reais da energia gerada pela usina com base na tarifa configurada.</p>
            <div className="mb-6 rounded-2xl bg-emerald-50 dark:bg-[#09120d] p-5 border border-emerald-100 dark:border-emerald-900/50">
               <p className="text-[10px] font-black uppercase text-emerald-800 dark:text-emerald-400">Tarifa utilizada no cálculo: R$ {solar.summary.tariffKwhBrl}/kWh</p>
            </div>
            <div className="grid grid-cols-3 gap-4">
              <div className="rounded-3xl bg-amber-50 dark:bg-amber-900/10 p-5 text-center">
                <p className="text-[9px] font-black uppercase text-amber-700">Valor Hoje</p>
                <p className="text-lg font-black">{cf.format(solar.summary.economyTodayBrl)}</p>
              </div>
              <div className="rounded-3xl bg-emerald-50 dark:bg-emerald-900/10 p-5 text-center">
              ...
              <div className="rounded-3xl bg-blue-50 dark:bg-blue-900/10 p-5 text-center">
                <p className="text-[9px] font-black uppercase text-blue-700">Valor Total</p>
          ...
          <section className="rounded-[2.5rem] bg-white dark:bg-[#0f1d15] p-8 border border-slate-200 dark:border-emerald-900/30">
            <div className="flex items-center justify-between mb-6">
              ...
              <p className="text-[10px] font-bold text-emerald-500 uppercase">Maravilha-AL | Atualizado em {new Date().toLocaleDateString()} {new Date().toLocaleTimeString().slice(0,5)}</p>
            </div>
            <div className="rounded-3xl bg-[#09120d] p-6 text-white border border-emerald-900/50">
               <div className="flex items-center justify-between">
                 <div>
                   <p className="text-[10px] font-black uppercase text-emerald-400 mb-1">AGORA EM MARAVILHA, AL</p>
                   <p className="text-6xl font-black">{weather?.current?.temp || "24,6"} °C</p>
                   <p className="text-xs font-bold uppercase opacity-80 mt-1">{weather?.current?.condition || "Predominantemente limpo"}</p>
                 </div>
                 <div className="text-right space-y-1">
                    <p className="text-[10px] font-bold opacity-60">Chuva: {weather?.current?.rainProb || "33%"}</p>
                    <p className="text-[10px] font-bold opacity-60">Vento: {weather?.current?.windSpeed || "9,7 km/h"}</p>
                 </div>
               </div>
               <div className="mt-8 grid grid-cols-3 gap-3 border-t border-emerald-900/40 pt-6">
                 {(weather?.forecast || []).map((f: any, idx: number) => (
                   <div key={idx} className="text-center p-2 rounded-xl border border-white/5 bg-white/5">
                     <p className="text-[9px] font-black uppercase text-emerald-400 mb-1">{f.day}</p>
                     <p className="text-[11px] font-bold leading-tight">{f.condition}</p>
                     <p className="text-[9px] opacity-60 mt-1">Máx {f.max}° | Min {f.min}°</p>
                   </div>
                 ))}
               </div>
            </div>
        ...
        <section className="rounded-[2.5rem] bg-white dark:bg-[#0f1d15] p-8 border border-slate-200 dark:border-emerald-900/30">
          <h2 className="text-xl font-black mb-2 dark:text-white">Status dos inversores</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-8">Resumo por equipamento para operação e acompanhamento.</p>
          <div className="grid gap-6 md:grid-cols-2">
            {(solar?.inverters || []).map((inv: any) => (
              <div key={inv.id} className="rounded-[2rem] bg-slate-50 dark:bg-[#0d1a12] p-6 border border-slate-100 dark:border-emerald-900/20">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <p className="text-lg font-black text-emerald-900 dark:text-white uppercase">{inv.name}</p>
                    <p className="text-[9px] font-mono opacity-50 tracking-tighter">{inv.serial || "5060KMTO243W000X"}</p>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="p-3 rounded-2xl bg-amber-50 dark:bg-amber-900/10 border border-amber-100/50">
                    <p className="text-[8px] font-black uppercase text-amber-700 mb-1">Potência</p>
                  ...
                  <div className="p-3 rounded-2xl bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100/50">
                    <p className="text-[8px] font-black uppercase text-emerald-700 mb-1">Hoje</p>
                  ...
                  <div className="p-3 rounded-2xl bg-blue-50 dark:bg-blue-900/10 border border-blue-100/50">
                    <p className="text-[8px] font-black uppercase text-blue-700 mb-1">Acumulado</p>
